...
fn server_url(state: tauri::State<'_, AppState>) -> ServerUrlResponse {
  ...
  match guard.as_ref().map(DshHandle::status) {
    Some(DshStatus::Ready(url)) => ServerUrlResponse { url: Some(url), error: None },
    Some(DshStatus::Failed(error)) => ServerUrlResponse { url: None, error: Some(error) },
...
fn main() {
  ...
  tauri::Builder::default()
    ...
    .setup(|app| {
      ...
      tracing::info!("运行时来源: {:?}, dsh 入口: {}", runtime.source, runtime.dsh_entry.display());
      ...
      .expect("注册信号处理器失败");
      app.manage(AppState { dsh, runtime, dsh_home });
      Ok(())
    })
